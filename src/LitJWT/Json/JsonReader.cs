...
namespace Utf8Json
{
    ...
    internal ref struct JsonReader
    {
        ...
        public JsonReader(ReadOnlySpan<byte> bytes, int offset)
        {
            this.bytes = bytes;
            this.offset = offset;
        ...
        JsonParsingException CreateParsingException(string expected)
        {
            ...
            try
            {
                ...
                switch (token)
                {
                    case JsonToken.Number:
                        ...
                        actual = Encoding.UTF8.GetString(ns);
                        break;
                    case JsonToken.String:
                        //actual = "\"" + ReadStringSegmentRaw() + "\"";
                        break;
                    case JsonToken.True:
                        actual = "true";
                        break;
            ...
            return new JsonParsingException("expected:'" + expected + "', actual:'" + actual + "', at offset:" + pos, pos, offset, actual);
        }
        ...
        JsonParsingException CreateParsingExceptionMessage(string message)
        {
            ...
            return new JsonParsingException(message, pos, pos, actual);
        }
        ...
        public ReadOnlySpan<byte> ReadStringSegmentRaw()
        {
            ReadOnlySpan<byte> key = default(ReadOnlySpan<byte>);
            if (ReadIsNull())
            {
            ...
            else
            {
                ...
                OK:
                key = bytes.Slice(from, offset - from - 1);
            }
        ...
        public ReadOnlySpan<byte> ReadNumberSegment()
        {
            SkipWhiteSpace();
        ...
        // return last offset.
        static int ReadComment(ReadOnlySpan<byte> bytes, int offset)
        {
            // current token is '/'
            if (bytes[offset + 1] == '/')
            {
    ...
    internal class JsonParsingException : Exception
    {
        ...
        public JsonParsingException(string message, int offset, int limit, string actualChar)
            : base(message)
        {
            this.Offset = offset;
